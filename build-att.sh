...
export INITRAMFS_SOURCE=`readlink -f ..`/jf/Ramdisks/AOSP_ATT
...
export CROSS_COMPILE=/home/cphelps76/kernel/linaro/bin/arm-eabi-
...
make VARIANT_DEFCONFIG=jf_att_defconfig SELINUX_DEFCONFIG=jfselinux_defconfig SELINUX_LOG_DEFCONFIG=jfselinux_log_defconfig gummy_jf_defconfig

HOST_CHECK=`uname -n`
if [ $HOST_CHECK = 'cphelps76-HP-Pavilion-dv7' ] || [ $HOST_CHECK = 'Ubuntu-VirtualBox' ]; then
	echo "cphelps76/kejar31!"
	make -j24
else
...
cp Packages/03gummycpuclock $PACKAGEDIR/system/etc/init.d/03gummycpuclock
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	rm ramdisk.gz
	rm zImage
	zip -r ../GummyKernel-JFatt-$curdate.zip .
	cd $KERNELDIR
else
